...
public petDto findById(int id) { //특정 정보를 가져오라고 시킬때 정보 조회
    ...
    public Slice<petEntity> findByNameContains(PetRequestDto requestDto, Pageable pageable) { //검색한 입력의 단어를 꺼낸다
        Slice<petEntity> list = this.petRepositoryy.findByNameContains(requestDto.getSearchName(), pageable);
        //Repository 에게 이름이 포함된 아이를 찾아라 라고 명령한다
        //이때 pageable(규격집 매개변수는 컨트롤러에서 보낸 사이즈 페이징 정렬 기준들의 정보)정한다
        return list;
    }
}

//Slice 이거와 Page 징의 차이 페이징을 데이터 가 천개 가 있으면 일일이 다 세고 Slice 이건 요청한 5개만 가져오ㅗ 뒤에 데이터가 더 있는지만 확인