...
//        System.out.println();
    ...
    public void Reduction() {
        List<Sstudent> sstudentList = Arrays.asList(
                new Sstudent("홍길동", 10),
                new Sstudent("신용권", 66),
                new Sstudent("집갈래", 23)
        );

        int sum1 = sstudentList.stream()
                .mapToInt(Sstudent::getScore)
                .sum();
        int sum2 = sstudentList.stream()
                .map(Sstudent::getScore)
                .reduce(0, (a, b) -> a + b);
    ...
    public void CollectE() {
        List<eotjfs> eotjfsList = new ArrayList<>();
        eotjfsList.add(new eotjfs("홍길동", "남", 70));
        eotjfsList.add(new eotjfs("김수영", "여", 11));
        eotjfsList.add(new eotjfs("김자바", "남", 88));
        eotjfsList.add(new eotjfs("오해영", "여", 78));

        List<eotjfs> male = eotjfsList.stream()
        ...
        Map<String, Integer> map = eotjfsList.stream()
                .collect(
                        Collectors.toMap(
                                s -> s.getName(),
                                s -> s.getScore()
    ...
    public void CollectEefe() {
        ...
        Map<String, List<eotjfs>> map = eotjfsList.stream()
                .collect(
                        Collectors.groupingBy(s->s.getSex())
                );

        List<eotjfs> maleList = map.get("남");
        maleList.stream().forEach(s->System.out.println(s.getName()));
        System.out.println();

        List<eotjfs> femaleList = map.get("여");
        femaleList.stream().forEach(s->System.out.println(s.getName()));

    }

    //요소병렬처리
    public void ParallelE(){
        Random random = new Random();

        List<Integer>scorse = new ArrayList<>();
        for(int i=0; i<1000000;i++){
            scorse.add(random.nextInt(101));
        }
        ...
        Stream<Integer> stream =scorse.stream();
        ...
        Stream<Integer>parer = scorse.parallelStream();
        startTime =System.nanoTime();
        avg =parer
                .mapToInt(i-> i.intValue())
                .average()
